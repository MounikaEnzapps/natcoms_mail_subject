    ...
    def _notify_record_by_email(self, message, recipients_data, msg_vals=False,
        ...
        if message.model == 'account.move':
            if self.system_inv_no:
                if self.so_number:
                   # mail_subject = self.system_inv_no or (self.system_inv_no and 'Re: %s' % message.record_name) # in cache, no queries
                    mail_subject = str(self.system_inv_no)+"  "+"for"+"  " + self.so_number+" "+"-"+"  "+self.partner_id.name
                else:
                    mail_subject = str(self.system_inv_no)+"  "+"for"+"  "+self.partner_id.name
            else:
